...
for(i in idp_types) {
  ...
  fn_jpeg = paste0(tempdir(), stringr::str_replace(basename(fn), '\\.pdf', '\\.jpeg'))
  pdftools::pdf_convert(fn, filenames = fn_jpeg, verbose = F, dpi = 300, format = "jpeg")
  cat(paste0('![](', fn_jpeg, ')'))
}
```
...
if(!is.null(df_mr)) {
  ...
  for(i in mr_idps$IDP) {
    ...
    cat(msg, '\n\n')
    fn_jpeg = paste0(tempdir(), stringr::str_replace(basename(fig_fn), '\\.pdf', '\\.jpeg'))
    pdftools::pdf_convert(fig_fn, filenames = fn_jpeg, verbose = F, dpi = 200, format = "jpeg")
    cat(paste0('![](', fn_jpeg, ')'))
  }
} else {
