...
if(!is.null(df_mr)) {
  xcandf_w_mr %>% select(IDP, modality, subtype, region, side, zscore, !!sym(bxcan_pval_col), `IDP -> Phenotype`, `Phenotype -> IDP`, link) %>% filter(!!sym(bxcan_pval_col) < p_thres) %>% filter(modality=="dMRI") %>% arrange(!!sym(bxcan_pval_col)) %>% select(-modality) %>% head(ntop) %>% pander
} else {
...
if(!is.null(df_mr)) {
  xcandf_w_mr %>% select(IDP, modality, subtype, region, side, zscore, !!sym(bxcan_pval_col), `IDP -> Phenotype`, `Phenotype -> IDP`, link) %>% filter(!!sym(bxcan_pval_col) < p_thres) %>% filter(modality == "T1") %>% arrange(!!sym(bxcan_pval_col)) %>% select(-modality) %>% head(ntop) %>% pander
} else {
  cat('not shown since no significant IDP being identified (pass p-value cutoff).\n\n')
}
```
