...
qval_cutoff = params$qval_cutoff
...
if(any(xcandf$qvalue < qval_cutoff) {
  p_thres = min(xcandf[[bxcan_pval_col]][xcandf$qvalue < qval_cutoff])
  z_thres = abs(qnorm(p_thres / 2))
} else {
  p_thres = 0
  z_thres = NULL
...
Among `r n_pred_idps` IDPs (`r sum(xcandf$modality=="T1")` T1 and `r sum(xcandf$modality=="dMRI")` diffusion MRI, including the subtype-specific PCs), `r sum(xcandf$qvalue < qval_cutoff)` were significantly associated with `r phenotype_name` (FDR < $`r qval_cutoff`$). 
...
if(!is.null(df_mr)) {
  xcandf_w_mr %>% select(IDP, modality, subtype, region, side, zscore, qvalue), `IDP -> Phenotype`, `Phenotype -> IDP`, link) %>% filter(qvalue < qval_cutoff) %>% filter(modality == "dMRI") %>% arrange(qvalue) %>% select(-modality) %>% head(ntop) %>% pander
} else {
...
if(!is.null(df_mr)) {
  xcandf_w_mr %>% select(IDP, modality, subtype, region, side, zscore, qvalue), `IDP -> Phenotype`, `Phenotype -> IDP`, link) %>% filter(qvalue < qval_cutoff) %>% filter(modality == "T1") %>% arrange(qvalue) %>% select(-modality) %>% head(ntop) %>% pander
} else {
  cat('not shown since no significant IDP being identified (pass p-value cutoff).\n\n')
}
```
